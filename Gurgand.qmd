...
link to the GitHub : https://github.com/LeonG000/grades-eval 
...
failure_by_course <- grades_long |>
  filter(avg_grade < 5) |>
  group_by(course) |>
  summarise(n_failures = n(), .groups = "drop") |>
  arrange(desc(n_failures))

most_responsible_course <- failure_by_course |> slice_head(n = 1)
